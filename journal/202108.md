...
Welcome to new first time contributors with code merged to master: {@handle} ([{repo}]({link to user's commits})), ... ! {NONE THIS MONTH}

Congratulations to new contractors granted the Decred Contractor Clearance (DCC): [@vibros68](https://github.com/vibros68). { hint: check the CMS }
...
**Hashrate**: August's [hashrate](https://dcrdata.decred.org/charts?chart=hashrate&zoom=krrqjgnz-kt1oo9gz&scale=linear&bin=block&axis=time) opened at ~315.8 Ph/s and closed ~359.5 Ph/s, bottoming at 163.7 Ph/s and peaking at 412.5 Ph/s throughout the month.

Distribution of hashrate [reported](https://miningpoolstats.stream/decred) by the pools on Sep 1: Poolin 36.1%, F2Pool 28.5%, AntPool 15.98%, BTC.com 7.84%, Easy2Mine 5.06%, Luxor 3.27%, ViaBTC 1.59%, HuobiPool 1.28%, OKEx 0.14%, CoinMine 0.08%, UUPool 0.07%.
...
**Staking**: [Ticket price](https://dcrdata.decred.org/charts?chart=ticket-price&zoom=krrqjgnz-kt1oo9gz&axis=time&visibility=true-true&mode=stepped) varied between 120.08-321.5 DCR, with 30-day [average](https://dcrstats.com/) at 193.80 DCR (+3.1).
...
The ticket price went down to 120 DCR and shot up to an ATH of 321.5 DCR. The staking participation also reached a new ATH of 61.1%. 
...
Node versions as of Sep 1 [snapshot](https://nodes.jholdstock.uk/user_agents) (252 dcrd nodes): v1.6.2 - 55.55%, v1.6.0 - 13.88%, v1.6.1 - 11.5%, v1.7 dev builds - 13.09%, v1.6 dev builds - 2.38%, v1.5.2 - 1.98%, v1.5.1 - 0.39%.
...
A new VSP [synergy-crypto.net](https://vspd.synergy-crypto.net/) has been [added](https://github.com/decred/dcrwebapi/pull/147).
...
In August DCR was trading between USD 126.5-181.26 / BTC 0.003208-0.003807. The average daily rate was $161.24.

{ markets overview: USD and BTC prices, highs and lows, interesting events, interesting analysis }

{ dcrdex updates, esp. volume }

Dcrdata now lists [trading](https://explorer.dcrdata.org/market?chart=candlestick&xc=dcrdex&bin=1h) and [volume](https://explorer.dcrdata.org/market?chart=depth&xc=dcrdex&bin=1h) data for the BTC/DCR pair in the DCRDEX.
...
A16Z, a VC with stakes in many crypto networks, has [published](https://a16z.com/2021/08/26/open-sourcing-our-token-delegate-program/) its "Token Delegate Program", explaining why and how they delegate their (substantial) voting rights in the various projects they have invested in. Around half of their stake seems to be delegated to various University-based organizations, with the rest being delegated to startups and non-profits. 

A Uniswap proposal from Flipside crypto caused some [controversy](https://twitter.com/DuneAnalytics/status/1428250340139356164) in the community as it breezed through early rounds of voting, it would have used $25M worth of UNI to generate yield that paid the Flipside team to produce "community-enabled analytics". One of the issues raised was that the entities named to oversee the grant included people who would benefit from it. All of the other providers of analytics were unimpressed by the attempt to get UNI top fund 8 full time employees for Flipside.
