...
- Implemented initial version of the [Pages](https://github.com/companyzero/bisonrelay/pull/224) feature. Pages allow to browse static content from a remote user. Only [Markdown]({ markdown guide }) pages are supported.
- Implemented initial [simple Stores](https://github.com/companyzero/bisonrelay/pull/224). Unlike Pages, Stores are dynamic resources that can present product items for sale. Store has a front page and individual product pages that are generated from local files. Store is reloaded automatically if any of its files changes. Peer users can browse products, add them to cart and place an order. Prices are shown in USD but are paid in DCR. Exchange rate is sourced from [dcrdata](https://explorer.dcrdata.org). Placed order is valid for 60 minutes and DCR amount does not change. CLI app only supports on-chain payments, while GUI app supports both LN and on-chain. { _for GUI too?_ }
- Pages and Stores are big features that will need rounds of improvements in response to user feedback. Limitations of the first iteration are listed in the [pull request](https://github.com/companyzero/bisonrelay/pull/224).
  - { _dig how it works? how "purchases" work? also remove limitations reference if they were fixed_ }
...
- Added `circuitlimit` config parameter which limits the number of [open connections](https://github.com/companyzero/bisonrelay/pull/234) when using proxy (such as Tor).
...
- Enabled brclient to delegate requests to a HTTP or HTTPS [website](https://github.com/companyzero/bisonrelay/pull/224). This allows BR users to access HTTP/HTTPS resources.
