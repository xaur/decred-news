...
- Fixed some issues with [text selection](https://github.com/companyzero/bisonrelay/pull/226), added max height to code blocks and made them scrollable. Two Flutter widgets have been forked and patched to workaround the issues, but double click (select word) and triple click (select paragraph) still don't. { _check linked issues_ }
