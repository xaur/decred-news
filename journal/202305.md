...
[Release v1.7.7](https://github.com/decred/dcrd/releases/tag/release-v1.7.7) (April 7, 2023) is the latest version, with no new backports in May.
...
- @Bee [discovered a bug via dcrdata](https://github.com/decred/dcrdata/issues/1963) where the total coin supply calculation returned from the dcrd RPC server didn't include the new TreasuryBases from the decentralized treasury. The total coin supply calculation [has now been updated](https://github.com/decred/dcrd/pull/3112), which also included an update to the database.
  - It should be noted that this bug was only a minor error with the RPC coin supply calculation, and did not affect consensus in any way.
- {TODO:} Implement DCP0011 PoW hash conensus vote
...
- { zec: implement fee-rate to short-circuit fee checks c8f7d91 }
- { summarize a streak of backports }
...
- Support Zcash-specific [fee calculation](https://github.com/decred/dcrdex/pull/2336). DEX calculates fees based on transaction size, but Zcash is an exception as it uses a standard fee of 1,000 zats or 0.00001 ZEC per transaction, regardless of size. Until [ZIP 317](https://zips.z.cash/zip-0317) activates to change this, the quick workaround is to set the fee rate such that all transactions pay at least 0.00001 ZEC. For v0.6.1 this is 10 zats per byte.
...
- Added [full node wallet](https://github.com/decred/dcrdex/pull/2270) support for [Firo](https://firo.org/) (formerly known as Zcoin). Up next is Electrum-based [light wallet](https://github.com/decred/dcrdex/issues/2346) support. { _follow-up fixes: 2353_ }
