...
- DEX v0.6.1 release with fixes and DigiByte support

- Politeia props, esp. Vanguard (may have its own bullet too)

- BR dev progress, first version of pages and stores
...
- Added compile time config for the [interval](https://github.com/decred/dcrdex/pull/2339) at which DEX server polls for new blocks of Bitcoin-like assets. It allows to use slower polling for remote nodes where the default 1-second interval would be too fast.
...
- Added a floating button to [jump to recent messages](https://github.com/companyzero/bisonrelay/pull/231). { _follow-up: 247_ }
...
- Fixed some issues with [text selection](https://github.com/companyzero/bisonrelay/pull/226), added height limit to code blocks and made them scrollable. Two Flutter widgets have been forked and patched to work around the issues, but double click (select word) and triple click (select paragraph) still don't work.
...
![](https://github.com/bochinchero/dcrsnapshots/raw/main/2023-05/700/Network_Monthly_Blocksize.png) { _upload_ }
...
- [CryptoWallet.com](https://cryptowallet.com/) features a mobile app that allows users to buy and sell crypto, supports VISA/MasterCard/SEPA transfers, and plans to launch own card product. The wallet is licensed in Estonia. Research and testing help is much appreciated.
...
{ _image: https://www.decredmagazine.com/ascend-to-greatness/ ._ }
...
- Decred Journal April 2023 was [translated](https://xaur.github.io/decred-news/) to Chinese (@Dominic) - thank you!
...
Community's DCR price analysis posted in #trading:

{ https://matrix.to/#/!lDZCzVQjFoJsXMPkvr:decred.org/$oxo3ofv0VnndaVIxA8MSsP5uvczTcz9pOyb1ED4ewoA }

_Image: Odd price wicks on DCR/USD seem to happen during low correlation with BTC/USD price action. Analysis by @saender._

{ https://matrix.to/#/!lDZCzVQjFoJsXMPkvr:decred.org/$Gd6oHLB8pDoA4j0keRGnmAFD5AM5Ye7lY_Pc3j8d53Y }

_Image: Market Valuations (USD) based on Decred-specific Staked Realised Value metric from @bochinchero_

{ https://matrix.to/#/!lDZCzVQjFoJsXMPkvr:decred.org/$1ivPF--zpiqDMBPtx4Q5-x1ro1cNb9IvUxuO26I9Rtk }

_Image: DCR/USD accumulation zones analysis by @saender_

![](https://github.com/bochinchero/dcrsnapshots/raw/main/2023-05/700/DEX_Monthly_Vol_USD.png) { _upload_ }
...
Bittrex filed for [bankruptcy](https://www.coindesk.com/policy/2023/05/08/us-crypto-exchange-bittrex-files-for-bankruptcy-in-delaware/) in Delaware but with a plan to reimburse all customer asset deposits, with the global Bittrex entity seemingly unaffected. { _does it mean only the U.S. branch filed for bankruptcy? any news/impact for Global? https://www.coindesk.com/policy/2023/06/14/bankrupt-crypto-exchange-bittrex-us-set-to-allow-withdrawals-starting-thursday/ ._ } In June Bittrex US was [allowed](https://www.coindesk.com/policy/2023/06/14/bankrupt-crypto-exchange-bittrex-us-set-to-allow-withdrawals-starting-thursday/) to re-open customer withdrawals, these would usually be frozen during liquidation but Bittrex argued to allow customers to withdraw their funds and a judge agreed, although stating that this did not make a determination that the customers (still) owned the assets and there may be clawbacks as the liquidation process continues.
