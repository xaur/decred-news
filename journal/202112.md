...
- introduced [`AssumeValid`](https://github.com/decred/dcrd/pull/2839) parameter and config option to skip certain validation checks prior to the specified block (which is assumed valid). Compared to using checkpoints for same purpose, `AssumeValid` allows to skip more validation work and gives greater flexibility to node operators and developers (for testing).
...
{ sanity check: [#2753](https://github.com/decred/dcrd/pull/2753) would trigger the hard fork vote for DCP0010 too right? }
