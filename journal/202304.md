...
- Many less exciting but necessary changes are made behind the scenes to support new assets more smoothly, such as code refactoring, cleanup, build code optimization, etc.
...
{ image candidates: some of 1993, 2313, shot of dex.decred.org with umbrel button }
...
- Improved UX for [embedding files in posts](https://github.com/companyzero/bisonrelay/pull/203). Several users missed to press the `+` button to add a file, resulting in posts without images. The confusing `+` button has been removed and files are now added in one action.
