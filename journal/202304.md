...
- Do not put GUI server's address into HTTP Content Security Policy header if it is an [IPv6 address](https://github.com/decred/dcrdex/pull/2287). { _reason?_ } Normally adding GUI server's IP or hostname to the CSP header should not be necessary in the first place, but the DEX does it to work around a [Safari bug](https://bugs.webkit.org/show_bug.cgi?id=201591).
...
- Default the GUI server to using [`127.0.0.1`](https://github.com/decred/dcrdex/pull/2283) IPv4 address when it is not specified in the config. This helps to satisfy the [Content Security Policy]({ mdn link }) protocol, which in turn is needed to workaround a bug in Safari. The new default avoids an issue where some [icons](https://github.com/decred/dcrdex/issues/1652) are not loading properly because of CSP failures. { _follow-up fix in 2295; simplify and/or group_ }
...
- Fixed spelling of [Zcash](https://github.com/decred/dcrdex/issues/2298) across the codebase.
...
- Umbrel ann https://twitter.com/decredproject/status/1648439889678192640 https://github.com/decred/dexweb/pull/34 { _worth its own ANN section? _}

In progress:

- { staking? https://github.com/decred/dcrdex/pull/2290 }
- { arb bot? https://github.com/decred/dcrdex/pull/2005 }
- { mm bot https://github.com/decred/dcrdex/pull/2302 2303  2304 2305 2306 2320 2332 }
- { Debian pkg https://github.com/decred/dcrdex/pull/1957 https://github.com/decred/dcrdex/pull/2308 }
- { Darwin https://github.com/decred/dcrdex/pull/2333 }
- Testers wanted for Debian, macOS and other OS-specific packages! Join the [#dexdev](https://chat.decred.org/#/room/#dexdev:decred.org) chat.
- { polygon https://github.com/decred/dcrdex/issues/2319 }
...
{ image candidates: some of https://github.com/decred/dcrdex/pull/1993 , 2313 }
