...
- Added a menu command to [suggest](https://github.com/companyzero/bisonrelay/pull/182) one user to do a key exchange with another user. This allows to connect two other users together by mediating their key exchange.
...
- Added tooltip explaining the [Cost](https://github.com/companyzero/bisonrelay/pull/196) field when sharing content (it is the price users will pay to access content).
...
- Improved UX for [embedding files in posts](https://github.com/companyzero/bisonrelay/pull/203). Several users missed to press the `+` button to add a file, resulting in posts without images. The confusing `+` button has been removed and files are now added in one action. Addition of alternative text has been moved to an optional modal.
- Added a page listing all [own posts](https://github.com/companyzero/bisonrelay/pull/204).
- Added [prepaid invites](https://github.com/companyzero/bisonrelay/pull/191). When such invite is imported its DCR will automatically fund the Bison Relay wallet of the invited user.
- Added account management (create and list accounts) and the ability to [send and receive DCR with on-chain](https://github.com/companyzero/bisonrelay/pull/191) (not LN) transactions. With this BR can act as a simple DCR wallet, making for a third actively supported GUI wallet after Decrediton and DCRDEX.
...
- Accept RVs that are marked as [paid](https://github.com/companyzero/bisonrelay/pull/193) but are not subscribed to by the client. This allows prepayment for RV subscriptions by other users and is used in prepaid invites feature. { _minir change, may group_ }
...
- During the reproducible building [sesssion](https://matrix.to/#/!zefvTnlxYHPKvJMThI:decred.org/$qjda4fjr5oQaDICsfnfhSRMmmoJtYx67zBrrRQqzyrk?via=decred.org&via=matrix.org&via=planetdecred.org) for v1.7.7 release developers encountered a mismatch in `go-ethereum` builds for DCRDEX, which turned out to be ["a doozy of a compiler bug"](https://github.com/golang/go/issues/59500) (it is quite an achievement to hit a compiler bug!) and another [bug](https://github.com/golang/go/issues/59525) in Go building code. Investigation has delayed the release by a few days, eventually both bugs got fixed and the fixes will ship in Go 1.21 and Go 1.20.4.
...
- Jay ([Decred Magazine](https://www.decredmagazine.com/author/jay/) author)
...
- Ticket pool value: 9,882,748.69 DCR
- Stake participation: 65.29% of circulating supply { _double check it's based on correct coin supply_ }
- Mixed and unspent DCR: 9,380,774.51 DCR
- Mixed and unspent %: 61.81% of circulating supply { _double check it's based on correct coin supply_ }
- LN nodes: 192
- LN channels: 392
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Network_Daily_Hashrate.png) { _upload_ }
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Daily_Hash_Dist.png) { _upload_ }
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Staking_Daily_TicketPrice_DCR.png) { _upload_ }

_Image: Ticker price has been unusually stable in April._
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Staking_Daily_TicketPoolVal_DCR.png) { _upload_ }

_Image: DCR locked in tickets is crawling up._
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Voting%20Service%20Provider%20(VSP)%20-%20Live%20Ticket%20Distribution.png) { _upload_ }
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Daily_NodeDistribution.png) { _upload_ }
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Stakeshuffle_Daily_MixedUnspent_DCR.png) { _upload_ }

_Image: Mixed supply is on the rise._

![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/Network_Monthly_TxTfrValAdjUSD.png) { _upload_ }

_Image: Uptick in [adjusted transaction](https://coinmetrics.io/introducing-adjusted-estimates/) volume in both DCR and USD._

{ _^ may drop if too much charts_ }
...
![](https://raw.githubusercontent.com/bochinchero/dcrsnapshots/main/2023-04/700/DCRLN_Daily_NodeCount.png) { _upload_ }

_Image: Decred's Lightning Network is seeing more nodes._
...
![](https://github.com/bochinchero/decred-pub/blob/main/MiningRewardBalance/all/Combined%20100k+%20PoW%20Block%20Reward%20Address%20Balance.png?raw=true) { _upload_ }

_Image: Combined balance of top 22 mining addresses vs DCR price history._
...
- New DCR/USDC, BTC/USDC and ETH/BTC pairs are [live](https://twitter.com/decredproject/status/1648190102802161671) at [dex.decred.org](https://dex.decred.org/). DEX client must be version 0.6.0 or higher in order to create ETH and USDC wallets and trade.
...
{ image candidates: Trocador screenshot, either one from #ecosystem or make another one showcasing their KYC detection }

Services lost or delisted:

- VSP [dcrpool.ibitlin.com](https://dcrpool.ibitlin.com) has been [removed](https://github.com/decred/dcrwebapi/pull/173) from the [VSP list](https://decred.org/vsp/) after being offline since Feb 28, last time it reported 4 live tickets. This VSP has served since Jan 2021 and voted ~1,200 tickets with a miss ratio of 0.75%. The fee was 1% of the ticket reward.

- VSP [dcrvsp.dittrex.com](https://dcrvsp.dittrex.com) has been removed from [VSPs](https://decred.org/vsp/), it was last seen online on Apr 04 with 2 live tickets. The owner [commented](https://github.com/decred/dcrwebapi/pull/175) on GitHub that the VSP is retired. It has served since Apr 2021 and voted ~500 tickets, missing only 0.6% of them. This one had a 5% fee.
...
- Discord is likely going to be shut down completely. Without the bridged content it may become a ghost town, and without enough moderators watching it will be taken over by spammers/scammers.
...
- [Decred Trading](https://t.me/DecredTrading) Telegram was renamed to [Bison Time](https://t.me/BisonTime). Decred branding has been removed to distance from negativity that is common in the channel and not scare away real investors who dare to join.
...
Join our [#ecosystem](https://chat.decred.org/#/room/#ecosystem:decred.org) chat to get more news from the Decred ecosystem.
...
- Pitched one news announcement 
- Pitched six media opportunities  

Secured the following media placements:

- An article in [Invezz](https://invezz.com/news/2023/04/17/decred-announces-dcrdex-0-6-with-support-for-ethereum-and-usdc-swaps/) about the launch of DCRDEX 0.6. The article was syndicated to five publications including [Bitcoin Insider](https://www.bitcoininsider.org/article/212399/decred-announces-dcrdex-06-support-ethereum-and-usdc-swaps).
- An article in [CoinJournal](https://coinjournal.net/news/decred-launches-dcrdex-0-6-the-latest-version-of-its-decentralized-exchange/) about the launch of DCRDEX 0.6. The article was syndicated to three publications including [Crypto News BTC](https://cryptonewsbtc.org/2023/04/17/decred-launches-dcrdex-0-6-the-latest-version-of-its-decentralized-exchange/). 
...
- @jy-p appeared on the [MineYourBiz Show](https://www.youtube.com/watch?app=desktop&v=aTaEul34RsA) talking about the current state of the Decred project and its new privacy features. 
...
- Coinbase sues SEC yay
  - https://www.theblock.co/post/228047/coinbase-sues-the-sec-for-answer-on-rule-specific-to-digital-assets
- EU regulation re mixing
  - not sure which new laws this is referencing and if it's April news
