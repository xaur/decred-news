...
- During the reproducible building [sesssion](https://matrix.to/#/!zefvTnlxYHPKvJMThI:decred.org/$qjda4fjr5oQaDICsfnfhSRMmmoJtYx67zBrrRQqzyrk?via=decred.org&via=matrix.org&via=planetdecred.org) for v1.7.7 release developers encountered a mismatch in `go-ethereum` builds for DCRDEX, which turned out to be ["a doozy of a compiler bug"](https://github.com/golang/go/issues/59500) (it is quite an achievement to hit a compiler bug!). This has delayed the release by a few days, eventually the bug was fixed in Go 1.21.
