...
The long-awated v0.6 has shipped around 8 months of development since v0.5. Major highlights:
...
- Standalone `dexc` app from [DCRDEX's own release](https://github.com/decred/dcrdex/releases) (has Windows built with no terminal window and system tray icon, signed by DEX developers)
...
New release has landed hust 4 weeks after v0.1.6 RC1. Notable changes in v0.1.7 final:

- Improved onboarding flows with new prepaid invites, server-based invites, and automated wallet setup
- Right-click on a username opens a menu with tipping and other useful commands
- Button to suggest a user to exchange keys with another user (i.e. introduce one to another)
- UI hints for new posts and new comments to existing posts
...
- Cleaned up and simplified public API of the internal [VSP code](https://github.com/decred/dcrwallet/pull/2227). This is part of consilidating VSP code in the [vspd](https://github.com/decred/vspd) repository for easier maintenance and code reuse.
...
- Fixed configured [VSP max fee](https://github.com/decred/dcrwallet/pull/2228) not being respected and always overridden with the hard-coded value of 0.2 DCR. Also, VSP config validation has been moved to startup code so that invalid options are reported earlier.
