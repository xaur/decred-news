...
The ASERT difficulty algorithm is also exponential, and can handle surging activity much better than the existing algorithm. This will greatly disincentivize "hit and run" miners who are only looking for a quick reward at a low difficulty. { _mention it also better handles *drops* of hashrate_ } The network is stronger when miners are consistently creating new blocks. [Reference discussion on Matrix](https://matrix.to/#/!uCnoeYfbhUnvwWihzS:decred.org/$tbF9LyO1EzQ8m3JDHfX9EYnQz1zr8W0Ra73Yjtprj1M). Fast math and bit tricks lovers, make sure to read the [ASERT code here]({link}).

{ _do we have the numbers how much faster ASERT will adjust to a sharp spike/sharp drop of hashpower, compared to EMA?_ }

{ _can we share the difficulty value immediately after it is reset? to make it tangible, what computing power will require to mine, say how many average modern CPUs?_ }
...
- Removed a [brittle test](https://github.com/decred/dcrwallet/pull/2253) that checked if the wallet DB was a specific version. { _1. it was an unhelpful test; 2. it tested db upgrade code; 3. may group (tests)_ }
