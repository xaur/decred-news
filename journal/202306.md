...
{ good stuff from https://matrix.to/#/!uCnoeYfbhUnvwWihzS:decred.org/$tbF9LyO1EzQ8m3JDHfX9EYnQz1zr8W0Ra73Yjtprj1M

> The new DAA (difficulty adjustment algorithm) is every block instead of every 144 now and it is exponential, which means it will adjust very quickly and discover the optimum difficulty within a few blocks.

> Unlike the existing algorithm though, the new one handles such surging activity much better and is no longer more profitable to be a switch hitter (aka hit and run miner).
