...
- Updated the [docker image](https://github.com/decred/dcrd/pull/3146) to build the dcrd binary with golang:1.20.5-alpine3.18. It's noteworthy to mention that the dcrd docker template doesn't persist the Alpine Linux OS. After building the binary, the OS is removed, and dcrd runs in a `scratch` docker container by itself. No OS, no root user, not even a shell. A lightweight container with maximum security.
...
- sync bytes and time
  - https://matrix.to/#/!zefvTnlxYHPKvJMThI:decred.org/$mbJD22XpiecybtWfRIhFRTawODG9wQujjoKSyOqegI0
  - what are these "Total Bytes"? memory?
...
- Updated to [Electron v23.3.8](https://github.com/decred/decrediton/pull/3884) from v18.3.15. Among other things, this gives devs the ability to let Decrediton communicate over USB, which is required for some hardware wallets like Ledger. Since WebUSB is built-in to the latest version of Electron, it means devs don't have to use external dependencies, which is good for security.
...
- BTC: Optimized the [initial deposit required for bonds](https://github.com/decred/dcrdex/pull/2392) to be much lower, ~0.004 BTC (~$100) versus the ~0.06 BTC (~$1,500). This will make it easier to onboard new users.
