...
- Updated integration test framework (dcrtest) and [re-enabled treasury tests](https://github.com/decred/dcrd/pull/3118). It was [temporarily disabled](https://github.com/decred/dcrd/issues/3093) to get around the circular dependency issue.
...
- Modified vote notification logic so that, after block height 777,240, [stakers will no longer approve blocks with old block versions](https://github.com/decred/dcrd/pull/3137). This is achieved by making dcrd not sent new block notifications to connected RPC clients, such as voting wallets. This change forces miners to upgrade to the latest block version in a timely fashion so that voting on the new consensus changes can commense.
...
DCP-11: Change PoW to BLAKE3 and a new difficulty algorithm using Absolutely Scheduled Exponentially weighted Rising Targets (ASERT) { _link to DCP-11 if available_ }
...
The ASERT difficulty algorithm is also exponential, and can handle surging activity much better than the existing algorithm. This will greatly disincentivize "hit and run" miners who are only looking for a quick reward at a low difficulty. ASERT also better handles [large drops](https://github.com/decred/dcrd/pull/3115#discussion_r1205741807) of hashrate. The network is stronger when miners are consistently creating new blocks and maintain the target block time of 5 minutes. [Reference discussion on Matrix](https://matrix.to/#/!uCnoeYfbhUnvwWihzS:decred.org/$tbF9LyO1EzQ8m3JDHfX9EYnQz1zr8W0Ra73Yjtprj1M). Fast math and bit tricks lovers, make sure to read the [ASERT code here](https://github.com/decred/dcrd/blob/d9b1921177d4626a22c380853d6c7030a2d5c854/blockchain/standalone/pow.go#L222).
...
- Updated to the latest [dcrtest](https://github.com/decred/dcrd/pull/3142), and excluded [unused modules](https://github.com/decred/dcrd/pull/3143) from regular builds.
