...
- Released new module versions as part of the regular pre-release development cycle: [addrmgr](https://github.com/decred/dcrd/pull/3121), [blockchain](https://github.com/decred/dcrd/pull/3133), [blockchain/standalone](https://github.com/decred/dcrd/pull/3120), [blockchain/stake](https://github.com/decred/dcrd/pull/3131), [chaincfg](https://github.com/decred/dcrd/pull/3125), [connmgr](https://github.com/decred/dcrd/pull/3124), [database](https://github.com/decred/dcrd/pull/3130), [dcrutil](https://github.com/decred/dcrd/pull/3129), [gcs](https://github.com/decred/dcrd/pull/3132), [hdkeychain](https://github.com/decred/dcrd/pull/3127), [peer](https://github.com/decred/dcrd/pull/3128), [rpcclient](https://github.com/decred/dcrd/pull/3134), [txscript](https://github.com/decred/dcrd/pull/3126), [wire](https://github.com/decred/dcrd/pull/3119), and the [main module](https://github.com/decred/dcrd/pull/3138). { _can add that other Decred software can now consume these new modules_ }
- Updated the [AssumeValid](https://github.com/decred/dcrd/pull/3122) block and [MinKnownChainWork](https://github.com/decred/dcrd/pull/3123) as part of the regular pre-release development cycle.

For reference, as of [May 2022](202205.md#development), `AssumeValid` has replaced the legacy "checkpoints" method to optimize initial blockchain downloads. With `AssumeValid`, very old blockchain forks are rejected, and some validation checks are skipped for blocks which are ancestors of the assumed valid block. This can significantly speed up initial blockchain downloads. The updates to `AssumeValid` and `MinKnownChainWork` are externally verified and hardcoded by devs, and used by default without any action required by node admins. (Note: It's possible to modify this functionality using the flags `--AllowOldForks` and `--AssumeValid {blockhash, 0}` as described in _dcrd/config.go_, but this is not recommended for typical node admins.) { _what does `MinKnownChainWork` do? can it be disabled like `AssumeValid` and what are the implications?_ }

- Updated [RPC integration tests](https://github.com/decred/dcrd/pull/3118). { _1. reword like "Updated integration test framework (dcrtest) and re-enabled treasury tests."; 2. also check 3093._ }
- If the majority of the network upgrades, miners will [generate new block versions](https://github.com/decred/dcrd/pull/3136), and  [nodes will reject old block versions](https://github.com/decred/dcrd/pull/3135). { _how about: "Miners running v1.8 will [generate new block versions](https://github.com/decred/dcrd/pull/3136) (10), and if the majority of the network upgrades [nodes will reject old block versions](https://github.com/decred/dcrd/pull/3135) (9)"_ }
- Modified the vote logic so that, after block height 777240, [stakers will no longer approve blocks with old block versions](https://github.com/decred/dcrd/pull/3137). This change forces miners to upgrade to the latest block version in a timely fashion so that voting on the new consensus changes can commense. { _1. "vote notification logic"; 2. add thousand separator to large numbers_ }
- [Removed module replacements](https://github.com/decred/dcrd/pull/3139), and [released 1.8.0](https://github.com/decred/dcrd/pull/3140). { _may group into the modules bullet_ }

DCP-11: Change PoW to BLAKE3 and a new difficulty algorithm using Absolutely Scheduled Exponentially weighted Rising Targets (ASERT) { _link to DCP-11_ }

Full PR with commits and comments [here](https://github.com/decred/dcrd/pull/3115), but the major changes included:

- Made the _block hash_ and _proof of work hash_ [separate and independent](https://github.com/decred/dcrd/commit/dbb6b782e0580395b3cb05333233994a70196ae7) so that each hash can be modified without changing the other. A great practical consequence is that _block hash_ consensus will remain unchanged with DCP-11, meaning that all software which handles block hashes will need no modification. Focused upgrades mean minimal downstream effects for consumers.
- Added [support](https://github.com/decred/dcrd/commit/66a59e0d9248632575d57cf26f5f338ed60ed9f5) for the [BLAKE3 hash function](https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE3) and split methods for computing the old PowHashV1 (BLAKE256) and the new [PowHashV2 (BLAKE3)](https://github.com/decred/dcrd/commit/b2878aa5c18a1a5669f769d594ec3fc4954aa779). Unfortunately, it's not possible to completely eliminate PowHashV1 and other old-consensus code. Even when blockchain consensus changes, it must remain possible to validate the old chain. Therefore, legacy concensus code is maintained to be able to validate the past 7 years of the Decred blockchain prior to newer consensus changes. { _mention lukechampine.com/blake3, also check the new cpuid dep and maybe mention it too_ }
- [Updated](https://github.com/decred/dcrd/commit/6c323c18b0eb7e4035c8ceb4ac3a36230c8c9c42) the `getwork` RPC and `notifywork` websocket notification. These are used by the mining code to fetch new data for hashing.
...
The ASERT difficulty algorithm is also exponential, and can handle surging activity much better than the existing algorithm. This will greatly disincentivize "hit and run" miners who are only looking for a quick reward at a low difficulty. { _mention it also better handles *drops* of hashrate_ } The network is stronger when miners are consistently creating new blocks. [Reference discussion on Matrix](https://matrix.to/#/!uCnoeYfbhUnvwWihzS:decred.org/$tbF9LyO1EzQ8m3JDHfX9EYnQz1zr8W0Ra73Yjtprj1M). Fast math and bit tricks lovers, make sure to read the [ASERT code here](https://github.com/decred/dcrd/blob/d9b1921177d4626a22c380853d6c7030a2d5c854/blockchain/standalone/pow.go#L222).

{ _do we have the stats how much faster ASERT will adjust to a sharp spike/sharp drop of hashpower, compared to EMA?_ }

{ _what will be the difficulty value immediately after the reset? to make it tangible, what computing power will require to mine, say how many average modern CPUs?_ }

![](https://user-images.githubusercontent.com/2115102/241322627-5f573007-2c96-4705-80ef-2f91ff432f0b.svg) { _upload_ }

_Image: Simulation of the ASERT difficulty algorithm reacting to stable, rapidly increaseing and rapidly decreasing hashrate._
...
- Bumped the master branch to [version 1.9.0-pre](https://github.com/decred/dcrd/pull/3141).
- Updated to the latest [dcrtest](https://github.com/decred/dcrd/pull/3142), and cleaned up [unused required modules](https://github.com/decred/dcrd/pull/3143). { "excluded unused modules from regular builds" }
- Updated [docker image](https://github.com/decred/dcrd/pull/3146) to golang:1.20.5-alpine3.18.
...
- Ensured that the wallet can correctly [validate either V1 or V2 PoW blocks](https://github.com/decred/dcrwallet/pull/2240), and updated the vote version. { _simplify V1 and V2 jargon_ }
- Prepared v1.8.0 release (and v3.0.0 module tag) by [bumping all Decred modules](https://github.com/decred/dcrwallet/pull/2241) to their latest versions and [updating the metadata](https://github.com/decred/dcrwallet/pull/2242). { _simplify_ }
- [Mixed ticket purchases](https://github.com/decred/dcrwallet/pull/2244) now have a random 20-60s delay before the wallet publishes each transaction to the mempool. This trickle effect will improve privacy by making it much harder for anyone surveilling the mempool to batch tickets together based on published timestamp.

The following work has been merged in `master` towards future releases:

- Removed mention of legacy [revocations](https://github.com/decred/dcrwallet/pull/2245). { _1. ticket revocations; 2. group or drop (too small)_ }
- Updated [.gitignore](https://github.com/decred/dcrwallet/pull/2246) to ignore test databases. { _may group (internal/dev)_ }
- Updated the [README](https://github.com/decred/dcrwallet/pull/2247) for people building from source.
- Bumped the major module version of the in-dev [dcrwallet module](https://github.com/decred/dcrwallet/pull/2248) to v4 to prepare for future changes. { _may group (internal/dev)_ }
- Brought dcrwallet up to date with VSPD, and [reduced dependencies between the VSP client and dcrwallet](https://github.com/decred/dcrwallet/pull/2238). { _may also mention the bigger goal of extracting vspd code_ }
- [Updated tests](https://github.com/decred/dcrwallet/pull/2250) to account for Go 1.21 changes. { _may group (tests)_ }
- Dropped [noisy logs](https://github.com/decred/dcrwallet/pull/2251) that were used for development. { _1. noisy TSpend logs; 2. only affected SPV mode; 3. may group (too small)_ }
- Removed a [redundant VSP pubkey check](https://github.com/decred/dcrwallet/pull/2255). { _may group (VSP refactor)_ }
- Removed a [brittle test](https://github.com/decred/dcrwallet/pull/2253) that checked if the wallet DB was a specific version. { _1. can say it was an unhelpful test; 2. can add that it tested db upgrade code; 3. may group (tests)_ }
- [Added new tests](https://github.com/decred/dcrwallet/pull/2254) for serializing VSP data { _reword "serializing", may group (tests)_ }
- Made the VSP client responsible for the [VSP logger](https://github.com/decred/dcrwallet/pull/2256), rather than using a global logger. { _may group (VSP refactor)_ }
- Cleaned up several VSP functions, as well as [improved how VSP client handles contexts](https://github.com/decred/dcrwallet/pull/2252). { _1. looks like a fix rather than an improvement; 2. may group (VSP refactor)_ }
...
- The VSP client doesn't need to handle [unmanaged (non-VSP) tickets](https://github.com/decred/dcrwallet/pull/2257). Moved the logic of unmanaged tickets out of the VSP client module and into the wallet module. { _clarify: VSP client does handle unmanaged ticket, but it doesn't need to *select* them from wallet's db_ }

{ _vspd extraction+refactoring has quite a few bullets now, may put them in their own section_ }
...
- Updated the [package.json](https://github.com/decred/decrediton/pull/3872) metadata for official publications, as part of the work done to add Decrediton to the [winget](https://en.wikipedia.org/wiki/Windows_Package_Manager) package repository. { _can link https://github.com/microsoft/winget-pkgs/pull/106493 ._ }
- Fixed an error that occurs when people break the connection between Decrediton and the DEX wallet. The default behavior when using the DEX from Decrediton is that the DEX simply uses the Decrediton wallet. However, it's possible for users to break that connection and force the DEX into SPV mode, which isn't ideal. The wallet will still work, but Decrediton can no longer control it properly and will throw errors in certain cases. This [patch fixes the error](https://github.com/decred/decrediton/pull/3866) by not allowing Decrediton to attempt to control a wallet that has been isolated to SPV mode by the DEX. However, users should still avoid telling the DEX to use SPV mode if it's already using the Decrediton wallet.
  - { _we can use more specifics from 3866. The root cause is people enabling SPV mode from within the DEX sub-app. It switches DEX to its own, entirely different wallet, and it stops using Decrediton's wallet. The code change is to only respect the password change command if the wallet belongs to Decrediton, and ignore it if the wallet is built-in. So Decrediton does not lose any control here._ } 
...
- Other minor updates and fixes including some typos, translation updates, and bumping dependencies. { 3863, 3856, 3868, 3849, 3875, 3877 } { _mention release notes in 3878_ }
- { _Updated PL translation deserves its own bullet_ }
- [Released v1.8.0](https://github.com/decred/decrediton/pull/3878)

The following work has been merged in `master` towards future releases:

- Minor changes like updating dependencies and removing old files. { 3876 (web dependencies) }
- Updated to [Electron v23.3.8](https://github.com/decred/decrediton/pull/3884). Among other things, this gives devs the ability to let Decrediton communicate over USB, which is required for some hardware wallets like Ledger. { _1. may mention previous Electron version; 2. mention WebUSB is needed to minimize deps_ }
- Updated to [DEX v0.6.2](https://github.com/decred/decrediton/pull/3882), which is required to follow the Decred 1.8.0 hard forks due to consensus changes. { _mention it fixes "response buffer too short" error reported by some users_ }
- Added an [Arabic translation](https://github.com/decred/decrediton/pull/3847). { _not a new translation so "Updated"_ }
- Replaced placeholder text for the [intro tutorials](https://github.com/decred/decrediton/pull/3881). This also adds proper release artwork for v1.8.

In progress:

- As mentioned [last month](202305.md#decrediton), work is ongoing to [add Ledger support](https://github.com/decred/decrediton/issues/3865), particularly the [Ledger UI](https://github.com/decred/decrediton/pull/3874). The most interesting new developments for June have been work to add the Ledger [backend functions](https://github.com/decred/decrediton/pull/3869) so that the UI can actually interact with the Ledger itself. The primary difficulty isn't simply verifying that the functions work as intended, but rather removing as many dependencies as possible to reduce the massive risk of dependency [supply chain attacks](https://en.wikipedia.org/wiki/Supply_chain_attack). Ledger software uses [numerous dependencies](https://npmgraph.js.org/?q=%40ledgerhq%2Fhw-app-btc), and if any of those dependencies were to be compromised by a bad actor, it could put wallet security at risk. Therefore, the Decred devs are taking great care to get this integration right. { _can link https://github.com/decred/decrediton/pull/3869#discussion_r1221483673 ._ }
...
- [Fixed a bug](https://github.com/decred/politeia/pull/1688) where the Politeia webserver would crash when trying to process a new user's registration payment if the transaction couldn't be found. Now, if the transaction cannot be found, the webserver will simply respond that the user hasn't paid yet. 
...
Changes that were included in the 1.2.0 release:

- Internal dev updates: updated [linter](https://github.com/decred/vspd/pull/384), refactored [database encoding](https://github.com/decred/vspd/pull/384), updated to use latest [dcrd](https://github.com/decred/vspd/pull/387) and [dcrwallet](https://github.com/decred/vspd/pull/388) [modules](https://github.com/decred/vspd/pull/392) in the client and in the [server](https://github.com/decred/vspd/pull/389), and Gin [web framework](https://github.com/decred/vspd/pull/391). { _dropped 385 (enough links for dep updates)_ }
...
Changes which have been merged to master towards future releases:

- Internal dev updates: bumped [dev version](https://github.com/decred/vspd/pull/394) and updated [.gitignore](https://github.com/decred/vspd/pull/396).
...
- Updated [CI config and dependencies](https://github.com/decred/dcrpool/pull/340) to use Go 1.20, PostgreSQL 15, dcrd and dcrwallet modules from v1.8.0, newer third party dependencies, and enabled additional code checkers (aka linters).
...
- Improved [startup time](https://github.com/decred/dcrlnd/pull/186) of dcrlnd and ported a few upstream changes to make it easier. The optimization is based on providing fast access to in-flight payments and removing the scan of all historical payments on startup.
- Fixed tests { 171 }.
...
- BTC: Optimized the [initial deposit required for bonds](https://github.com/decred/dcrdex/pull/2392) to be much lower, ~0.004 BTC ($100) versus the ~0.06 BTC (~$1,500). This was an issue when onboarding new users.
- Updated to latest dcrd and dcrwallet [modules](https://github.com/decred/dcrdex/pull/2403) from the v1.8.0 release.
- Enabled use of [external fees information](https://github.com/decred/dcrdex/pull/2391) for SPV wallets such as built-in DEX wallet or Electrum. This works for BTC-based assets: BTC, LTC, and BCH. Before this change it was possible to pay unreasonably [high fees](https://github.com/decred/dcrdex/issues/2354) in some scenarios.
- Optimized loading of [static GUI assets](https://github.com/decred/dcrdex/pull/2398) in the browser. CSS files, JavaScript files, images, and fonts will load faster.
...
- Workaround Badger database [truncation error](https://github.com/decred/dcrdex/pull/2389) which may happen on [Windows](https://github.com/dgraph-io/badger/issues/744). { _opt: explain what is "value log truncation" and if it may cause data loss_ }

All changes reported below are towards the next release.

Client:

- Added support for [HTTPS connection](https://github.com/decred/dcrdex/pull/2345) between client's front-end and back-end. When `dexc` is running on a public network it is enabled by default, otherwise it can be forced with `--webtls` flag. The browser will show a scary warning about a self-signed certificate but it can be overridden.
- Added [tootltips](https://github.com/decred/dcrdex/pull/2357) to explain various wallet balances (Available, Locked, Immature, Bond Reserves).
...
- Fixed a bug where [Accelerate button](https://github.com/decred/dcrdex/pull/2414) was showing for revoked (cancelled) trades.
...
- [Recycle unused](https://github.com/decred/dcrdex/pull/2368) redemption and refund addresses. DEX generates new Bitcoin addresses to be used for redemption (trade completed normally) and refund (trade was cancelled and refunded). Many addresses end up being unused. In Hierarchical Deterministic (HD) wallets this may create *gaps* between used addresses that are bigger than the [gap limit](https://blog.lopp.net/mind-the-bitcoin-address-gap/), which may cause problems with discovering funds and restoring wallets from seed. To avoid exceeding the gap limit, generated addresses that were not used for redemptions or refunds will not be thrown away and instead used for other trades.
...
- Implemented several low-level primitives and [test harness](https://github.com/decred/dcrdex/pull/2399) to aid in developing Polygon support. The harness was able to send funds on simnet and tesnet. Existing Ethereum code has been refactored to be more generic and reusable by Polygon code.
- Extended the [test harness](https://github.com/decred/dcrdex/pull/2413) to create two nodes, connect them, and mine blocks.

App packaging:

- Added packaging scripts to build a [DMG installer for macOS](https://github.com/decred/dcrdex/pull/2333).
- Created a [DCRDEX package](https://aur.archlinux.org/packages/dcrdex) in Arch User Repository (AUR). On Arch or Arch-based Linux distributions like Manjaro it can be installed with `yay` or `pamac` tools like `$ yay dcrdex`. Documentation has been [updated](https://github.com/decred/dcrdex/pull/2347) with a list of existing and upcoming packages for vairous operating systems.

Server:

- Changed order limits to be [based on bond strength](https://github.com/decred/dcrdex/pull/2411). Increasing the bond tier by locking more funds will unlock bigger order sizes. Order limit is also reduced by bad behavior like missing the swaps. This change should fix inability to submit orders for some users. The penalty and reward system is still a work in progress.

Developer and internal changes:

- Updated simnet blockchain with [new PoW](https://github.com/decred/dcrdex/pull/2358). { _group_ }
- Updated [linters](https://github.com/decred/dcrdex/pull/2375). { _group_ }
- Allow more flexible [golangci configuration](https://github.com/decred/dcrdex/pull/2393). { _group_ }
...
{ _image candidates: 2395 (bond help), 2383 (walle summary)_ }
...
- Updated [block rewards](https://github.com/decred/dcrdata/pull/1969) math to be aware of the DCP-12 activation date, as well the new block reward split defined in it.
...
- Testnet coin faucet updated to [Go 1.20](https://github.com/decred/testnetfaucet/pull/70) and [Decred modules](https://github.com/decred/testnetfaucet/pull/71) released with v1.8.0. Fauced is used by developers to obtain testnet DCR.
