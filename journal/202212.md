...
Bison Relay is a peer to peer social media platform with strong protections against censorship, surveillance and advertising. Messages are end-to-end encrypted to ensure only the intended recipients can see the contents. Server operators cannot see message contents, sender, receiver, or which chats user participates in. Built-in payments allow content monetization via tipping or paid access.
...
- Connecting with other users via invite files sent over existing channel, or via Bison Relay directly (your existing contacts connect you to their contacts)
  - { do mediated key exchanges work already? }
- Sending and receiving DCR tips via Decred Lightning Network
- Sharing files with custom cost per kilobyte that users pay to access { someone please check/expand this, e.g. does the "access cost" go to the server or to the poster? }
- Payments to the server for sending and receiving messages
- Creating and paying invoices
- { anything else to note here? }

{ tell how cheap to use it is }

Initial release includes the following software:

- Server
- Command-line client { ur is it a TUI? or both? }
- GUI client app or { platforms }
- { any other apps? }

{ ^ add download links for each + note on sigs missing temporarily }

{ directions how to join }
...
- [Setting up Bison Relay v0.1.1 on macOS](https://www.youtube.com/watch?v=L4jm-VMPmBo) video
...
To understand the motivation for creating Bison Relay check the posts by @jy-p:
...
- Initial [Bison Relay announcement](https://blog.decred.org/2022/12/14/Bison-Relay-The-Sovereign-Internet/)
...
- [Server]({ code }): Go, PostgreSQL, {}
- [Command-line client]({ code }): Go, {}
- [GUI client]({ code }): Go, Flutter, {}
