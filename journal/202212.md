...
- Refactored [login sequence](https://github.com/decred/dcrdex/pull/1903). Added a warning prompting to unlock the wallets ASAP to process active trades. { _what else does it fix?_ }
- Refactored [BTC fee estimation](https://github.com/decred/dcrdex/pull/1967) and added external DOGE fee estimator. External fee estimation is useful as a fallback for SPV (light) wallets and also RPC (full) wallets that have been started recently and their fee estimation has not calibrated yet. { _also read 1888_ }
