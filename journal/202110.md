...
- changed [trickling](https://github.com/decred/politeia/pull/1556) method (trickling is slow sending of votes). Previously it has sent votes in a sequence with random time intervals. This has proven to be brittle as one slow/failed send was delaying all other votes. The new method uses independent parallel voting processes that start at random times and do not affect each other, making trickling more robust.
...
Development continued despite the second [GoDCR proposal](https://proposals.decred.org/record/f7d9fc8) getting rejected (49% Yes). Look forward to the revised version and new app builds.
...
In response to a common question "What's up in Decred and where it is heading?" @bee wrote up a [summary](https://www.reddit.com/r/decred/comments/q1402a/weekly_many_musings_mondays/hfji878/) of recent developments and mid-term goals.
...
@tacorevenge published the [second part](https://medium.com/@tacorevenge/the-suppressor-part-2-on-chain-analysis-6561c5a478c4) of an investigation of The Suppressor entity that some community members suspect of manipulating DCR markets. This time on-chain analysis was used to see how funds have been flowing between centralized exchanges and DCRDEX.
...
Zcash has been polling its coinholders again, this time on the subject of whether to change the consensus mechanism away from Proof of Work. 85% of the 41,000 ZEC that voted put a switch from PoW as the number one priority for the project. { maybe add what % of supply the 41K ZEC is; also, is there a link? bonus: mention what alternatives they consider, if any at this point}
