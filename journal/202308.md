...
- Custom coin (output) selection for extra privacy
- Custom gap limit
...
Bug reports and feedback are welcome is [GitHub Issues](https://github.com/crypto-power/cryptopower/issues) or the [#cryptopower](https://matrix.to/#/!oxOZZtibVUXxXtdPJS:decred.org) Matrix chat.
...
- The document explains new proof of work hash, new ASERT difficulty adjustment algorithm, and the difficulty reset at the activation block, all in great detail with formulas, diagrams, and guidance for implementations.
- *Block header* stays exactly the same, it is a compact 180-byte structure that holds important metadata about the block and allows to sync and navigate the chain very quickly.
...
- ASERT is executed for every block which is 144x more often than the previous EMA algorithm. It it is also ~16x faster, so the overall dcrd will spend ~9x more time running ASERT code. However, this is on the order of microseconds per block and does not make any practical difference. Read more [here](https://github.com/decred/dcps/pull/29#discussion_r1300314310).
- DCP-11 is so far the biggest by the length of text, file count and overall size. It is very well made and all Decred enthusiasts are advised to [read the full thing](https://github.com/decred/dcps/blob/master/dcp-0011/dcp-0011.mediawiki).
...
- [Do not ignore](https://github.com/decred/dcrwallet/commit/1c1bcfbd5aa7ca0b1cbad06b4f1bb6b8c2af5aa5) VSP fee payment errors caused by the wallet being locked. Users should be warned if a locked wallet blocks VSP fee payments so they can react and prevent missed tickets.
...
- Start listening to [notifications from dcrd](https://github.com/decred/vspd/pull/407) after all other startup tasks have been completed. There was no problem with connecting to dcrd earlier but it was unnecessary and created messy logs.
...
Last time we wrote dev logs for this project in [August 2022](202208.md#godcr) when it was called GoDCR. This August was relatively quiet in terms of development while the team was focused on shipping the initial v1.0.0 release and working the the [proposal](https://proposals.decred.org/record/256efee) to get this project funded.
