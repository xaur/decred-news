...
Backported changes ready for the next v1.8.x release:

{ Note: I just copied and pasted these from the July issue. They're just backports from work done last month. }
...
{ Note: the item below is new this month. It's both in `master` and also got backported. Not quite sure how to handle this, but I'm putting it at the bottom of the "backported" section instead of the following section.}

- Fixed a bug where ticket purchase transactions could be sent to the VSP client before they were mined, resulting in the VSP not [calculating the block heights]() where the ticket would be live/expired. Without knowing if the ticket was live or expired, the VSP wouldn't vote on the ticket, and would result in a missed ticket. { _@jholdstock, is this true? What happened previously when `fp.ticketLive == 0` or `fp.ticketExpires == 0`? I can't figure out exactly what users would experience. Also, how often would unmined transactions reach the VSP? Seems like since the mempool travels faster than blocks, it would happen quite a lot, but VSPs don't miss many tickets._ }
