...
- Tidied up the Github Actions [build workflow](https://github.com/decred/dcrd/pull/3168) to reduce duplicate cache entries and cache spam. This involved removing the go module and build cache from the Github build workflow, since the latest `setup-go` action caches them by default, and this was creating duplicate cache entries. The linter is still cached separately, and this update also included a small filename change to produce significantly fewer cache objects. {dev}
- Updated the [Docker image](https://github.com/decred/dcrd/pull/3171) to build the dcrd binary with Go 1.21.0. {user}
- Updated the build and test actions to [support Go 1.21](https://github.com/decred/dcrd/pull/3172), and dropped official support for Go 1.19. This included updating the Github Actions build workflow `setup-go` and linter to their latest verisons. {user}
- Added a test scenario to the [ASERT tests](https://github.com/decred/dcrd/pull/3173) that ensures the difficulty adjusts appropriately when block times are speeding up. {user}
- Added a [duplicate-word linter](https://github.com/decred/dcrd/pull/3175), and used it to [tidy up comments](https://github.com/decred/dcrd/pull/3174) in 44 different files. {dev}
- General code cleanup such as [removing empty lines](https://github.com/decred/dcrd/pull/3176) {dev - gonna drop this}
- [Removed unused parameters](https://github.com/decred/dcrd/pull/3177) from a few blockchain functions: `parent` from the internal logger, and `isTreasuryEnabled` from `getStakeBaseAmounts`. These functions are internal, so this change has no impact to clients or users. {dev}
