...
- Added an [error linter](https://github.com/decred/dcrd/pull/3179), and updated all remaining unwrapped errors so that they can be [wrapped with useful context](https://github.com/decred/dcrd/pull/3178). This allows them to be unwrapped by function callers to identify the source of the error. This update allows the errors to work nicely with `errors.Is` and `errors.As`.
...
- Created a new logger that [logs transaction changes the wallet notices](https://github.com/decred/dcrwallet/pull/2275) during rescans. This logger is off by default during the initial scan, because it would be too noisy. However, during future rescans, the logger will announce when it finds new wallet transactions inside an old block, or wallet transactions which were previously unmined, but are now mined. Rescans aren't fun for users, and rescans are really just a bandaid to sync wallets that got out of sync and missed a transaction. This logger will be quite helpful to debug why a wallet had a problem syncing blocks and/or transactions, and will hopefully result in some good bugfixes that reduce user's need to rescan. {_user_}
...
- Moved the logic of [finding VSP managed tickets](https://github.com/decred/dcrwallet/pull/2259) out of the VSP client and into the main wallet code. This is part of the continued effort to refactor VSP and wallet logic. Additionally, fixed some error-handling logic and added new error logs if processing VSP tickets fails. {_user_}
