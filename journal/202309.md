...
## Decred GPU Mining Started

{ _quick howto info and links, link to #mining for the rest_ }
...
{ https://github.com/decred/dcrdex/releases/tag/v0.6.3 + better bins }
...
- Updates to expose the new [proof of work hash](https://github.com/decred/dcrd/pull/3192) in the verbose `getblock` and `getblockheader` results.
- Changed the [`getnetworkhashps`](https://github.com/decred/dcrd/pull/3181) treat `-1` as a default value (120 blocks back). Previous meaning of `-1` "the number of blocks since last difficulty change" no longer makes sence since the difficulty is now calculated every block as opposed every 144 blocks with the old difficulty algorithm.
...
- Minor changes { drop after review: 3186, 3189 }
...
Changes included in the v1.8.1 release:
...
- Updated [VSP fee calculation](https://github.com/decred/dcrwallet/pull/2281) for DCP-12 (where proof of stake reward increased from 80% to 89%).
  - { _if this is not backported v1.8.1, will VSPs accept unupdated fees?_ }

{ omitting minor: 2282, 2288 }
...
- Added [backend functions](https://github.com/decred/decrediton/pull/3869) for interacting with Ledger. This is using [WebUSB](https://github.com/electron/electron/pull/36289) API which allows Electron-based apps to access USB devices. Ledger is disabled on mainnet for now.
...
- Updated [DCRDEX module](https://github.com/decred/decrediton/pull/3905) to v0.6.3 and enabled its new "extension mode" to protect Decrediton's wallet from being mis-configured from DEX pages.
- Updated [Chinese translation](https://github.com/decred/decrediton/pull/3893).
- Updated web dependencies { 3898, 3894, 3891, 3890, 3904, 14:04
 }
- Fixes { 3900 (async), 3887 (perms) }
...
- Display separate [expired and missed ticket counts](https://github.com/decred/vspd/pull/417) on the VSP home page. Also, a new yellow banner at the top will appear if the VSP is running on testnet or simnet.
- Added separate [expired and missed ticket counts](https://github.com/decred/vspd/pull/421) to the `/vspinfo` response. This can be used by wallets and scripts to show more accurate VSP stats to the user.
- Updated [VSP fee calculations](https://github.com/decred/vspd/pull/435) to consider the new block reward split in DCP-12.
- Made [debug banner](https://github.com/decred/vspd/pull/443) red to be more noticeable because debug mode should not be used in production.

Changes visible to VSP admins:
...
- Reused code for [handling shutdown signals](https://github.com/decred/vspd/pull/438) sent by the user or the operating system, and improved compatibility with various OSes.
- Changed the web API to return [explicit errors](https://github.com/decred/vspd/pull/440) if the web API cache is not ready. This will make the admin immediately aware of a possible issue. Previously web pages were rendered with no data and no indication of anything being wrong.
...
- Unexported some code and [moved](https://github.com/decred/vspd/pull/428) it to [internal](https://github.com/decred/vspd/pull/430) [packages](https://github.com/decred/vspd/pull/436) since it is not suitable for consuming by third parties. Reduced public API surface makes development and maintenance cheaper.
...
- ~20 commits with smaller improvements and code cleanup.
  - { _drop after review: 433 (move), 429 (import kill), 432 (unparam), 431 (minor opti), 437 (rename), 441 (unused), 444 (x4 decomp, renames), 449 (deps), 446 (x8 minor cleanups and tweaks), 450 (ver)_ }
...
Similar to gominer, dcrpool development has reactivated after being mostly inactive since summer 2021 when release v1.2.0 came out. The majority of the work focused on upgrading the codebase to latest Go features, fixing multiple concurrency and shutdown edge cases, and paying some technical debt, all in preparation of adding pooled GPU mining support.

Upgrades and changes:

- Added [BLAKE3 support](https://github.com/decred/dcrpool/pull/341) to the internal CPU miner which is used for testing.
  - { _it is used for testing the pool right?_ }
- Improved accuracy when calculating the number of [possible iterations](https://github.com/decred/dcrpool/pull/360).
- Improved tracing of [clients that time out](https://github.com/decred/dcrpool/pull/376) in the logs.
...
- Check if the app is shuttingdown before sending [internal messages](https://github.com/decred/dcrpool/pull/367). { _may group (shutdown fixes)_ }
...
- [Fail fast](https://github.com/decred/dcrpool/pull/377) if the GUI web server's list port is already in use. Before this change, if the GUI could not start normally dcrpool process would continue to run, which is not an expected behavior. Now the process will terminate quickly and allow the admin to resolve any issues immediately.
- Fixed edge cases where duplicate [work share IDs](https://github.com/decred/dcrpool/pull/390) and [payment IDs](https://github.com/decred/dcrpool/pull/392) could be generated. Random numbers have been added to these IDs to ensure they are unique. This has fixed test failures on fast hardware.
- Miscellaneous concurrency, cancellation, and shutdown fixes { 375 (client goroutine leak), 394 (paymgr goroutine leak), 395 (process pmts goroutine leak), 396 (paymgr races) } { _maybe name some kinds_ }
- Fixed potential panic during [shutdown](https://github.com/decred/dcrpool/pull/397) { _may group with shutdown fixes_ }
- Miscelanneous error handling fixes { 354, 372 }
...
- Made various code cleanups and enabled new linters { _comments and errors (343, 344, 345, 346, 347), unused code (350, 379), formatting (362), deprecations (393)_ }
...
- Removed all code related to ASICs that can no longer mine Decred and are no longer supported by dcrpool. { 398, 399, 400 }
...
- Removed custom configuration to [target 100 peer connections](https://github.com/decred/dcrlnd/pull/190) [inherited](https://github.com/lightningnetwork/lnd/blob/d233f61383f2f950aa06f5b09da5b0e78e784fae/server.go#L1413) from lnd.
- Fixed [startup failure](https://github.com/decred/dcrlnd/pull/191) if Routing is not configured in the config file or a CLI switch.
- Build and test against [Go 1.21](https://github.com/decred/dcrlnd/pull/192).
- [v0.4.0 release](https://github.com/decred/dcrlnd/releases/tag/v0.4.0) has been created to mark the revision included in core software v1.8.0 release.
...
- [Extension mode](https://github.com/decred/dcrdex/pull/2486), a restricted mode where DCRDEX won't allow to change settings like wallet type, wallet password, or wallet accounts. This will protect the wallet from mis-configuration when the DEX is running as part of Decrediton.
- Removed ["bonus tiers"](https://github.com/decred/dcrdex/pull/2502) from server's account tier calculation (they were not doing anything).

Work below has been merged to `master` towards future releases.

Decred:

- Added minimal [GUI for VSP staking](https://github.com/decred/dcrdex/pull/2482) with the following features: select a VSP, view current ticket price and voting rewards, purchase tickets, view all-time staking stats, view a list of owned tickets, view and set voting preferences (consensus consensus agendas, treasury spends, and treasury keys).
...
- Implemented Firo [block parsing](https://github.com/decred/dcrdex/pull/2488) to enable fee estimation on testnet (on mainnet and simnet fee estimates can be obtained from Firo software).
- Minor { 2494 (harness), 2495 (docs and typos), 2506 }

Ethereum:

- Implemented [WebSocket subscriptions](https://github.com/decred/dcrdex/pull/2490) so the DEX server could get new block headers from chain data provider more efficiently than by polling them. When HTTP polling is still used, poll interval will automatically increase for non-local sources. Previously this could only be controlled with a compile-time flag.
- Extended the list of [free chain data servers](https://github.com/decred/dcrdex/pull/2514) used in testing { _may group_ }
...
- Changes across the board to complete [Polygon support](https://github.com/decred/dcrdex/pull/2431): generalized the Ethereum wallet and server backend to reuse common code, added RPC compatibility tests, added Polygon token support to the test harness and gas estimation, deployed USDC swap contract for Polygon mainnet, updated token-related UI code. Polygon and Ethereum wallets will use the same seed and will derive the same private keys and addresses as [discussed here](https://github.com/decred/dcrdex/pull/2431#issuecomment-1650510000).
...
- Improved [active orders UI](https://github.com/decred/dcrdex/pull/2171): show a fixed size table with live orders followed by recent orders, and show buttons when hovering an order so the user could perform actions without expanding the rows.
- Show [average traded rate](https://github.com/decred/dcrdex/pull/2463) in several places instead of just "market" or zero.
...
- Added interfaces to retreive the [history of wallet transactions](https://github.com/decred/dcrdex/pull/2500) (actual implementation will be done in future changes).
- Allow the client to connect to DEX servers using certificates provided by trusted [Certificate Authorities](https://github.com/decred/dcrdex/pull/2513) which is common for [TLS connections](https://en.wikipedia.org/wiki/Transport_Layer_Security#Certificate_authorities). Previously server certificate had to be configured manually.
- Optimized [exchange rates fetching](https://github.com/decred/dcrdex/pull/2512) logic and timeouts.
- Fixed the possibility of unexpected [bond expiration](https://github.com/decred/dcrdex/pull/2460) and cancellation of all orders. Client uses account tier information from the server to decide how many bonds to queue up. If the client is targeting tier 1 and has no good behavior recorded by the server it will create a new bond before the one runs out to stay bonded. However if the server tells the client they are tier 2 because of good behavior, the client will assume it does not need to create a new bond when the last bond is about to expire, and when that happens, the tier will suddenly drop to 0, cancelling all orders.
...
- Updated [market making UI](https://github.com/decred/dcrdex/pull/2491) to reflect the latest changes to how it works. The UI includes an overview page with all configured market makers and settings pages for each. The configuration is stored in a JSON file. The markets page is also updated to not allow users to place orders manually while market making is running.
- Fixed the market maker bot to [rebalance](https://github.com/decred/dcrdex/pull/2517) when the necessary matches data is available.

Documentation:

- Updated [built-in BTC wallet recovery and rescanning](https://github.com/decred/dcrdex/pull/2473) guide.
- Added [release notes](https://github.com/decred/dcrdex/pull/2536) for v0.6.3.
- { omitting: 2516, 2533, 2539 }
...
- Added an integrated [reverse tunnel](https://github.com/decred/dcrdex/pull/2493) for obtaining chain data services. Instead of DEX server connecting to data providers, they can initiate connections to a public DEX server and provide RPC services. This allows obtaining chain data from private nodes that cannot accept connections. Having an integrated reverse tunnel offers some advantages over SSH tunneling used previously to achieve the same goal, such as simple configuration. DEX server can consume from multiple source nodes for redundancy.
...
- Added a script for manually testing with a 2-wallet pair](https://github.com/decred/dcrdex/pull/2535) on mainnet and testnet. This allows to conveniently test more scenarios.

{ omitting: 2519, 2528, 2509, 2518, 2546 }
...
- Implemented new [Home page](https://github.com/crypto-power/cryptopower/pull/65) design and reorganized features into 3 high-level tabs Overview, Wallets and Trade. (work in progress, some parts are not functioning yet? ) { 65 (dummy tabs), 68 (layout fixes), 75 (impl with dummy data) }
...
- Show [loading indicator](https://github.com/crypto-power/cryptopower/pull/73) while VSPs are being loaded.
- Hide [mixing progress bar](https://github.com/crypto-power/cryptopower/pull/70) when mixer is not running.
- Persist [hidden balance](https://github.com/crypto-power/cryptopower/pull/81) setting across app restarts.
- Added a modal for [receiving funds](https://github.com/crypto-power/cryptopower/pull/108) that shows upon clicking the Receive button, which is always visible. The modal shows an address and a QR code for the specified wallet account and amount.
...
- Fixed width of the [asset balance card](https://github.com/crypto-power/cryptopower/pull/95) on the Overview tab.
...
- Fixed crash when loading the app to the [Overview page](https://github.com/crypto-power/cryptopower/pull/133) when loading to the Overview page.
...
- Added [initial Kotlin project](https://github.com/crypto-power/cryptopower-android).
...
- Removed unused code { 66 }
- Minor fixes { 96 (readme) }

Bug reports and feedback are welcome via [GitHub Issues](https://github.com/crypto-power/cryptopower/issues) or the [#cryptopower](https://matrix.to/#/!oxOZZtibVUXxXtdPJS:decred.org) Matrix chat.
...
- Added the [exchanges](https://github.com/decred/dcrweb/pull/1139) Kucoin and Huobi. Removed Hotbit. { _move to Eco_ }
